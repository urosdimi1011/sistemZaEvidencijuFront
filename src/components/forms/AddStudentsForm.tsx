...
import { Checkbox, Field, Radio, RadioGroup, Textarea } from '@fluentui/react-components';
...
export default function AddStudentsForm({ onSuccess, student } : AddStudentFormProps) {
    ...
    const [schoolSelect,setSchoolSelecte] = useState(null);
    ...
    const schema = yup.object({
        ...
        skola: yup.string().required(),
        createdAt: yup.string().nullable(),
        procenatManagera: yup
            .number()
            .nullable()
            // 👇 Uslovna validacija - procenat je obavezan ako je menadžer izabran
            .when('managerId', {
                is: (managerId: number | null) => managerId !== null && managerId !== undefined,
                then: (schema) => schema.required('Procenat menadžera je obavezan kada je menadžer izabran'),
    ...
    useEffect(() => {
        const hasSchool = watchSelektovanaSkola !== null && watchSelektovanaSkola !== undefined;
    ...
    }, [watchSelektovanaSkola, schoolSelected, trigger]);
    ...
    const getOccupationsFromSchool = async ()=>{
        try{
            ...
            const filteredOccupationsForDropdown = response.data.occupations.map((s : any)=>{
                return {
                    label : s.name,
                    value : s.id
                }
    ...
    const getOccupations = async ()=>{
        try{
            const endpoint = isSchoolManager && userSchoolId
    ...
    useEffect(()=>{
            ...
            try{
                ...
                await getSchools();
                // Postavi početno stanje za menadžera
                if (student) {
                    const hasManager = student.menadzer?.id !== null && student.menadzer?.id !== undefined;
                    setManagerSelected(student.menadzer?.id !== null && student.menadzer?.id !== undefined);

                    if(student.schoolId) {
                        setSchoolSelecte(student.schoolId);
                ...
                setError("Greska kod dohvatanja menadzera ili zanimanja: "+xhr.message);
            }
        })()
    },[])
    ...
    return (
        <>
            <form key={student?.id || "new-student"} onSubmit={handleSubmit(onSubmit)}>
                ...
                <MyInput autoFocus {...register('ime')} control={control}  label={'Unesite ime ucenika'} name={'ime'} className='my-1'></MyInput>
                {errors.ime && <div className="text-red-900 text-shadow">{errors.ime.message}</div>}

                <MyInput {...register('imeRoditelja')} control={control} label={'Unesite ime roditelja'} name={'imeRoditelja'} className='my-1'></MyInput>
                {errors.imeRoditelja && <div className="text-red-900">{errors.imeRoditelja.message}</div>}

                <MyInput {...register('prezime')} control={control} label={'Unesite prezime ucenika'} name={'prezime'} className='my-1'></MyInput>
                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <MyDropdown
                            ...
                            placeHolder={schoolSelected ? 'Izaberite zanimanje učenika' : 'Prvo izaberite školu'}
                            id="skolaId"
                            name="zanimanje"
                            options={zanimanja}
                            value={field.value}
                ...
                {errors.type && <div className="text-red-900">{errors.type.message}</div>}
                ...
                {errors.entry_type && <div className="text-red-900">{errors.entry_type.message}</div>}


                <MyInput type='number' {...register('cenaSkolarine')} control={control} label={'Unesite Cenu skolarine za studenta'} name={'cenaSkolarine'} className='my-1'></MyInput>
                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <MyDropdown
                            ...
                            placeHolder={managerSelected ? 'Izaberite procenat za menadzera' : 'Prvo izaberite menadžera'}
                            name="procenatManagera"
                            id="procenatManagera"
                            value={field.value}
                ...
                <button className='mt-10 mb-5' type="submit" disabled={loading}>
                    {loading ? 'Sačekajte...' : student !== undefined ? 'Edituj učenika' : 'Dodaj učenika'}
                </button>
            </form>
             {showConfirmModal && (
